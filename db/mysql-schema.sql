...
-- Optional: mapping existing Animal.breed (free text) to normalized breed_id in future
-- ALTER TABLE Animal ADD COLUMN breed_id INT NULL,
--     ADD CONSTRAINT fk_animal_breed_id FOREIGN KEY (breed_id) REFERENCES Breeds(breed_id) ON DELETE SET NULL;

-- ======================
-- 10. Veterinarians Table
-- ======================
...
-- ======================
-- 11. HealthRecords Table (general health visits/diagnosis)
-- ======================
...
-- ======================
-- 12. Feeds Table (catalog of feed types)
-- ======================
...
-- ======================
-- 13. CowFeed Table (animal-to-feed consumption using catalog)
-- ======================
CREATE TABLE IF NOT EXISTS CowFeed (
    cow_feed_id       INT AUTO_INCREMENT PRIMARY KEY,
    animal_id         VARCHAR(20) NOT NULL,
    ...
    FOREIGN KEY (animal_id) REFERENCES Animal(animal_tag) ON DELETE CASCADE,
    FOREIGN KEY (feed_id) REFERENCES Feeds(feed_id) ON DELETE RESTRICT
);
...
-- ======================
-- Seeding for new normalized tables
-- ======================

-- Breeds
INSERT INTO Breeds (breed_name, description) VALUES
('Friesian', 'High-yield dairy breed'),
('Jersey', 'Small dairy breed with rich milk'),
('Ayrshire', 'Hardy dairy breed'),
('Sahiwal', 'Tropical dairy breed'),
('Guernsey', 'Golden milk breed');

-- Veterinarians
INSERT INTO Veterinarians (full_name, phone, email, license_number, notes) VALUES
('Dr. Kiptoo', '0700000001', 'kiptoo@vet.org', 'VET-001', NULL),
('Dr. Wambui', '0700000002', 'wambui@vet.org', 'VET-002', NULL),
('Dr. Mwangi', '0700000003', 'mwangi@vet.org', 'VET-003', NULL);

-- HealthRecords (link some existing animals and vets)
INSERT INTO HealthRecords (animal_id, visit_date, diagnosis, treatment, outcome, veterinarian_id, notes) VALUES
('A002', '2023-08-15', 'Mastitis', 'Antibiotics 10ml', 'Improved', 1, 'Follow-up in 7 days'),
('A007', '2023-08-16', 'Hoof injury', 'Painkiller 5ml', 'Recovered', 3, NULL);

-- Feeds catalog
INSERT INTO Feeds (feed_name, unit, cost_per_unit, description) VALUES
('Napier Grass', 'kg', 50.00, 'Fresh chopped fodder'),
('Dairy Meal', 'kg', 120.00, 'Commercial concentrate'),
('Hay', 'kg', 60.00, 'Dry forage'),
('Silage', 'kg', 80.00, 'Fermented forage'),
('Molasses', 'kg', 40.00, 'Energy supplement');

-- CowFeed examples using catalog
INSERT INTO CowFeed (animal_id, feed_id, quantity, unit, date, cost_override) VALUES
('A002', 1, 15.0, 'kg', '2023-08-01 08:05:00', NULL),
('A003', 2, 10.0, 'kg', '2023-08-01 08:35:00', NULL);
